...
from column_lists3 import (gap_quant, non_gap_merge)
...
def draw_weather_boxplots(averages):
    ...
    for key in obs_keys:
        f.write('<g>\n	<text x="' + str(h_base + h_step / 2) + '" y="' + str(v_offset + reference * 2) + '" text-anchor="middle">' + key + '</text>\n')
        v_scaler = reference / smart_units(v_scaler_dict[key])
        print "DEBUG: {0}: {1}. {2}, {3}".format(key,v_scaler,reference, smart_units(v_scaler_dict[key]))
        f.write('<text x="' + str(h_base + h_step / 2) + '" y="' + str(v_offset + reference - 2) + '" text-anchor="middle">' + str(v_scaler_dict[key]) + '</text>\n')
        total = 0
        for m in range(12):
...
def draw_simple_summary(summary, name):
    y = 0
    h_scale = 150
    bar_height = 4 #16
    text_shift = 13
    bar_h_spacing = 30
    bar_v_spacing = 4 #30
    f = open(output_root+'/Visuals/RGA/Summary/summary_{0}.svg'.format(name), 'w')
...
def completeness_calendar(merged, column):
    n = len(merged['data'])
    if n > 0:
        Years, year_0, scaler, space_between_years, f, SVG_rect_template, start_year, year_incr = initialize_svg_calendar(merged['Years'],'{0}_{1}_completeness'.format(merged['name'], column))
        print "  Drawing completeness calendar for {0} column {1}: ".format(merged['name'], column)
        ...
        for hour in merged['data']:
            ...
            this_day = hour['datetime'].timetuple().tm_yday
            year = hour['datetime'].timetuple().tm_year
            if hour[column] != 'none': case = 'data'
            else: case = 'empty'
            if case == previous_case and this_day == previous_day: repeat_count += 1
            else:
                if not first_datapoint:
                    write_svg_rect(f, previous_day * scaler, upper_hour * scaler + start_year - year_offset,
                                   repeat_count * scaler, previous_case)
...
def draw_daylight(dawn_dusk, name): #Not sure how this will handle high latitudes where there are days with no dawn or dusk.
...
def generate_column_statistics(merged, P_list = [0,0.01,0.05,0.15,0.25,0.5,0.75,0.85,0.95,0.99,1.0]):
    empty_values = ['','None',None,'none']
    columns = gap_quant + non_gap_merge
    value_lists = collections.OrderedDict([(column,[]) for column in columns])
    for row in merged['data']:
        for column in columns:
            if column in row and row[column] not in empty_values:
                try:
                    value = float(row[column])
                    value_lists[column].append(value)
                except: pass
    all_keys = []
    for P in P_list:
        for column in columns:
            all_keys.append('{0}_{1}'.format(column,P))
    output_row = collections.OrderedDict([('Station',merged['name'])]+[(key,None) for key in all_keys])
    for column in columns:
        percentiles = Percentiles(value_lists[column],P_list)
        if percentiles != 'none':
            for i, P in enumerate(P_list):
                key = '{0}_{1}'.format(column,P)
                output_row[key] = percentiles[i]
    return output_row
