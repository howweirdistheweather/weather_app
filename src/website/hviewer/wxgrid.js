...
function LoadReadingDropdown() {
    ...
    const co_url = 'wxapp/readingtypes.json'
...
function LoadAggDropdown() {
    ...
    const co_url = 'wxapp/aggmethods.json'
    fetch( co_url, {    method:'GET',
                        headers: {'Authorization': 'Basic ' + btoa(cred_str)}
                    }
    )
...
const co_url = 'wxapp/stationsmeta.json'
fetch( co_url, {    method:'GET',
                    headers: {'Authorization': 'Basic ' + btoa(cred_str)}
                }
)
...
function LoadStates( current_country ) {
    ...
    const state_url = `wxapp/stationsmeta.json?sm_type=co&sm_co=${current_country}`
...
function LoadStations( current_country, current_state ){
    ...
    const st_url = `wxapp/stationsmeta.json?sm_type=state&sm_co=${current_country}&sm_state=${current_state}`
...
function LoadWXGrid( station_id, reading_type, agg_method ) {
    ...
    RenderGrid();
    // AJAX the JSON
    var wxgrid_url = `wxapp/plot0.json?df_station=${station_id}&df_col=${reading_type}&df_method=${agg_method}`;
